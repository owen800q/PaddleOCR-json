...
// 套接字 (POSIX sockets - same as Linux)
#include <unistd.h>
...
namespace PaddleOCR
{
    ...
    // macOS uses Mach APIs for memory info instead of /proc filesystem
    int Task::get_memory_mb()
    {
        try
        {
            ...
            {
                std::cerr << "Error: task_info() failed with error code " << kerr << std::endl;
                return -1;
            }
        ...
        {
            std::cerr << "Error: macOS get_memory_mb" << std::endl;
            return -1;
        }
    }

    // 代替cv imread，接收utf-8字符串传入，返回Mat。
    // macOS uses UTF-8 natively, same implementation as Linux
    ...
    // Socket server implementation using POSIX sockets (same as Linux)
